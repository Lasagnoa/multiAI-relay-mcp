...
1つのプロセス内でアクティブなプロジェクトは常に1つだけです。

| 状況 | 問題 |
|------|------|
| Claude Desktop の同じウィンドウで別プロジェクトに切り替えた | それ以前のチャットも新しいプロジェクトを参照するようになる |
| 複数チャットで同じ Desktop を共有している | どちらかの `collab_switch_project()` がもう一方に影響する |

**推奨構成（複数プロジェクトを並行する場合）:**
